...
function isPalindrome(str) {
  ...
  const middle = Math.floor(newStr.length / 2) - 1;

  for (let i = 0, j = newStr.length - 1; i <= middle; i += 1, j -= 1) {
...
function encodeToRot13(str) {
  ...
  function encoder(char, from, to, shift) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    let char = encoder(str[i], 97, 122, 13);
    char = encoder(char, 65, 90, 13);
